...
class ChatSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Chat
        fields = ['id', 'title', 'is_group_chat', 'users', 'created_at', 'messages', 'profiles', 'unread_messages_count']
    ...
    def get_unread_messages_count(self, obj):
        # Assuming `Message` has a field `is_read` to track if a message is read
        user = self.context['request'].user
        unread_messages = Message.objects.filter(chat=obj, is_read=False).exclude(sender=user)
